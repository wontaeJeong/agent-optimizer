...
export default async () => ({
  config: async (config) => {
    ...
    if (Boolean(endpoint) === Boolean(base)) throw new Error("Set exactly one of MODEL_ENDPOINT or MODEL_BASE_URL");
    ...
    const model = process.env.MODEL_ID ?? "glm5.3-flash";
    if (!model || /\s/.test(model)) throw new Error("MODEL_ID must be a nonempty model identifier");
