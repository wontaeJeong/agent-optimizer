...
보고서의 기존 한국어/영어 선택과 재생성 시 언어 보존을 따른다. 그래프의 범례·SVG 접근성 설명·확정적 insight도 같은 언어로 표시하며 원본 candidate/task ID와 metric 이름은 번역하지 않는다. 인쇄에서는 시각화와 최종 테스트·실패 요약·출처만 남기고, 원시 평가표·중복 후보/단계 상세는 브라우저 화면과 원본 JSON에 남긴다.
