...
| `Registry` | 패키지 내장 알고리즘·하네스와 **명시적으로 선택된** 파일 플러그인만 해석한다. 설치된 CLI가 `cwd`에서 저장소의 모든 예제를 찾지 않는다. 개발자가 `experiments/<team>/`에 등록하는 ID→구현 파일 경로와 실험별 `[plugins.*]`는 유지한다. 알 수 없는 ID/중복 등록은 실패한다. |
