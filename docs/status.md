...
**2026-09-22 반복 데모:** OpenAI 호환 endpoint/Bearer 설정(기본 `glm5.3-flash`), 기본 3회
LLM 피드백 Optimizer, priority encoder train/QAM16 validation, setup 사전검사와 구조화 doctor를 추가했다.
...
실제 배포 모델의 OpenCode→모델→CVDP live inference는 모델 설정·인증이 필요하다.
...
| 단순 LLM 피드백 | `experiments/simple-feedback/optimizer.py`, `models.py` | 기본 3회 train 피드백→지침 수정·평가. 파일 플러그인 교체 가능. 로컬 API fixture와 실제 runner로 반복/선택/사용량 검증, 연구 알고리즘 아님. |
...
`experiment.toml`은 `simple_feedback` 파일 플러그인으로 `references/role-guidance.md` 안내 후보를 만든다.
adapter가 내용을 prompt에 직접 포함한다. 이번 데모는 OpenCode 스킬 프로필로 진행하기로 결정했다.
실환경 검증 후 주장할 수 있는 것은 선택한 데이터·모델·예산에서 **이 프로필의 안내 변경 효과**다.
스킬을 읽는 실행만으로 ACE-RTL 전체 알고리즘·native 역할 코드 최적화나 논문 재현을 주장할 수 없다.
현재는 실제 효과 자체도 미검증이다. 이 프로필 선택은 이번 데모의 범위이며 모든 Agent의 고정 실행 방식은 아니다.
...
- setup은 고정 HF full no_commercial JSONL을 받아 302개 중 71개를 지원 형태로 변환하고
  231개 제외 사유를 기록한다. live는 priority encoder train + QAM16 validation, evaluator-only smoke는 별도 고정 repo LFSR 예제다.
...
- importer 기본 변환은 validation이며, demo selector가 명시한 두 과제를 train/validation으로 분리한다.
  `final_test=false`다. 작은 데모로 일반화 성능을 판단할 수 없다.
