...
2026-09-22 데모 범위 결정: OpenCode의 ACE 스킬 프로필, 교체 가능한 단순 LLM 피드백
Optimizer부터 시작한다. 기본 3회 train 탐색 후 별도 validation으로 선택하며 연구 알고리즘은
팀이 공통 계약의 파일 플러그인으로 추가한다. 아래 대표 프로필 미결정은 장기 비교 실험에 해당한다.
모델 설정은 OpenAI 호환 API와 환경별 모델 ID를 사용한다. TUI·resume·병렬 실행은 이번 범위에서 제외한다.
