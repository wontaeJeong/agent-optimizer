...
source snapshot → baseline validation → **각 Optimizer를 baseline에서 독립 실행**
→ stage별 validation winner → 기본적으로 모든 stage winner 비교 → 선택 고정 → 선택적 test → 보고서.
Optimizer의 `evaluate/history`는 train 전용이다. baseline train 측정은 cache를 공유할 수 있지만
...
| `config.py`, `registry.py` | TOML·과제·호환성 검사, 내장 및 명시적 `file.py:Symbol` 등록 |
...
| `objectives.py`, `results.py` | lexicographic keep=1·mean/sum 집계, JSONL/Markdown/CSV |
...
후보 `candidate.json`/`changes.diff`, trial 로그/`result.json`, `events.jsonl`,
`frozen_selection.json`, `summary.json`/`report.md`를 보존한다. seed가 모든 backend의 결정성을 보장하지는 않는다.
...
summary 상태는 `completed`, `no_eligible_candidate`, `budget_exhausted`, `interrupted`,
