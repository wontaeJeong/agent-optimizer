...
1. `make setup ARGS="--core"` 후 `agent-opt datasets list` 또는 `agent-opt tui`에서
...
3. **팀 플러그인 하나:** [역할 선택](../experiments/README.md) 후 팀 폴더/`extensions.toml`을 고쳐
   `datasets list --extensions ...`와 `plan`하고 구현한다.
