...
`sh scripts/bootstrap.sh setup`부터 실행하세요. 메뉴 자체는 시작 시 설치·진단·모델 호출·결과
생성을 하지 않습니다. `make menu ARGS="--help"`는 비대화형 환경에서도 도구 조회 없이 동작합니다.

| 번호 | 동작과 준비 조건 |
|---|---|
| 1. 개발 환경 설치 | 기존 `setup` 실행. 아래 Git·Docker·Compose 사전 조건과 다운로드/빌드 시간이 적용됩니다. |
| 2. 환경 진단 | 기존 `doctor` 실행. 설치나 모델 호출 없이 준비 상태와 복구 방법을 표시합니다. |
| 3. LLM 없이 데모·최적화 반복 테스트 | 준비된 프로젝트 `.venv`에서 실제 최소 합성 데모 후 `test_feedback_optimizer.py` 회귀 테스트를 실행합니다. 로컬 HTTP fixture를 사용하며 외부 LLM·Docker가 필요 없습니다. 실제 모델 최적화나 성능 검증은 아닙니다. 코어만 준비하려면 `uv sync --frozen --python 3.12 --extra dev`를 사용하세요. |
| 4. 모델 설정·연결 검사 | 정확한 endpoint 또는 표준 base URL을 고르고 모델 ID·숨김 Bearer 토큰을 입력한 뒤 **실제 `doctor --model` 호출**. 모델 서비스와 준비된 Docker/예제 환경이 필요합니다. |
| 5. ACE 최적화 실행 | 설정한 모델로 **실제 `live --iterations N` 호출**. 기본 3회, 1..20회만 허용하며 전체 setup과 모델 설정이 필요합니다. 미설정이면 4번을 안내합니다. |
